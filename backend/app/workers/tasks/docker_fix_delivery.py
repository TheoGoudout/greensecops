...
def _build_docker_pr_body(fixes: list[DockerFix]) -> str:
    lines = [
        f"## {settings.PROJECT_NAME} Docker fixes",
        "",
        "This PR applies automated fixes to Dockerfiles and Compose files "
        f"flagged by {settings.PROJECT_NAME} static analysis.",
        "",
    ]
...
def deliver_docker_fixes(
    ...
    with Session(engine) as session:
        ...
        if repo.installation_id is None:
            logger.warning(
                "Docker fix delivery skipped for repo %s: no installation",
                repo.full_name,
            )
        ...
        fixes = list(
            session.exec(
                ...
                .where(
                    DockerFix.status == FixStatus.ready
                    if not force
                    else DockerFix.status.in_(  # type: ignore[attr-defined]
        ...
        if (
            ...
            logger.info(
                "Skipping docker delivery: PR on branch %s was closed", pr_branch
            )
        ...
        try:
            ...
            logger.exception("Failed to fetch docker files for delivery: %s", exc)
        ...
        pr_title = f"fix(docker): apply {settings.PROJECT_NAME} fixes to {target.root_path or '/'}"
        pr_body = _build_docker_pr_body(fixes)
