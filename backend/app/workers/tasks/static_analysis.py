...
def _run_static_analysis_impl(
    ...
    with Session(engine) as session:
        ...
        # re-analysed off stale content, and doing it *before* the dedup check is
        # what keeps a dedup-skipped file's stored copy from going stale at all.
        ...
        # sync resolves the branch head itself, so a task delayed behind the lock
        # or a retry can no longer write the commit its event happened to carry
        # over newer content that landed since.
        try:
        ...
        # repo has no workflows". Recording a `no_workflows` analysis here would
        # state as fact the one thing we just established we cannot tell, and it
        # would show in the history as though the repo had been emptied. Say
        # nothing and let the next push, poll or nightly run settle it.
        ...
        # The commit actually analysed, which is what belongs on the Analysis
        # row. Falls back to the trigger's SHA only when the ref could not be
        # resolved, so the column is populated for manual and scheduled runs too.
        ...
        for path in analysed_paths:
            if budget is not None and budget <= 0:
                ...
                quota_stopped_at = path
                break

            # The content the sync just read at ``sync.head_sha``, never the
            # row's ``raw_content``: the two are the same except when this run's
            # write lost the ordering race, and there the row is the newer one.
            content = sync.contents[path]
            ...
            # The row the sync created or refreshed for this path. Persisting
            # content is the sync's job, not the analysis loop's — it used to
            # happen here, below the dedup `continue` above, which is exactly
            # why a dedup-skipped file's stored copy went stale.
            wf_record = sync.rows[path]
        ...
        if quota_stopped_at is not None:
            ...
            skipped = len(analysed_paths) - len(results)
            message = (
