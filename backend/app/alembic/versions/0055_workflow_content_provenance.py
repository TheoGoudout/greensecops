...
`workflow_fix.base_content` is the file content a rewrite was generated *from*, and
`workflow_fix.base_commit_sha` the commit it came from. A fix replaces the whole file,
...
def upgrade() -> None:
    ...
    op.add_column("workflow_fix", sa.Column("base_content", sa.Text(), nullable=True))
    op.add_column(
        "workflow_fix", sa.Column("base_commit_sha", sa.String(length=40), nullable=True)
    )


def downgrade() -> None:
    op.drop_column("workflow_fix", "base_commit_sha")
    op.drop_column("workflow_fix", "base_content")
