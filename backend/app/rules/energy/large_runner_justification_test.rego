...
test_violation_gpu_runner_no_heavy_workload if {
	violations := large_runner_justification.violations with input as {"jobs": {"ci": {
		"runs-on": "ubuntu-gpu",
		"steps": [
...
test_no_violation_large_runner_with_build_step if {
	violations := large_runner_justification.violations with input as {"jobs": {"compile": {
...
test_no_violation_standard_runner if {
	violations := large_runner_justification.violations with input as {"jobs": {"test": {
...
test_no_violation_gpu_runner_with_train_step if {
	violations := large_runner_justification.violations with input as {"jobs": {"ml": {
