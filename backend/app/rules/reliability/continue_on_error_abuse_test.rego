...
test_violation_step_with_continue_on_error if {
	violations := continue_on_error_abuse.violations with input as {"jobs": {"build": {"steps": [{
		"name": "Flaky step",
...
test_no_violation_no_continue_on_error if {
	violations := continue_on_error_abuse.violations with input as {"jobs": {"build": {"steps": [{"name": "Build", "run": "make build"}]}}}
	count(violations) == 0
}
