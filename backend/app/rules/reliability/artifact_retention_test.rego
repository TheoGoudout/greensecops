package greensecops.reliability.artifact_retention_test

import data.greensecops.reliability.artifact_retention
import rego.v1

test_violation_upload_without_retention if {
	violations := artifact_retention.violations with input as {"jobs": {"build": {"steps": [{
