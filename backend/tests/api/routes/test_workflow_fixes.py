...
def pending_workflow_file(db: Session, repo: Repository) -> WorkflowFile:
    """A second workflow file (one WorkflowFix per workflow file — unique constraint)."""
    wf = WorkflowFile(
        repo_id=repo.id,
...
@pytest.fixture()
def pending_fix(db: Session, pending_workflow_file: WorkflowFile) -> WorkflowFix:
    f = WorkflowFix(
...
def test_get_fix_found(
    ...
    # Act
    response = client.get(
        f"{settings.API_V1_STR}/workflow/fixes/{ready_fix.id}",
        headers=superuser_token_headers,
    )
...
def test_get_fix_not_found(
    ...
    # Act
    response = client.get(
        f"{settings.API_V1_STR}/workflow/fixes/{uuid.uuid4()}",
        headers=superuser_token_headers,
    )

    # Assert
    assert response.status_code == 404
    assert response.json()["detail"] == "Workflow fix not found"
...
def test_list_pull_requests_returns_repo_prs(
    ...
    # Arrange — a PR row exists but no WorkflowFix currently references it (the
    ...
    # Act
    response = client.get(
        f"{settings.API_V1_STR}/workflow/repositories/{repo.id}/pull-requests",
        headers=superuser_token_headers,
    )
...
def test_list_pull_requests_repo_not_found_returns_404(
    ...
    # Act
    response = client.get(
        f"{settings.API_V1_STR}/workflow/repositories/{uuid.uuid4()}/pull-requests",
        headers=superuser_token_headers,
    )
...
def test_generate_fixes_for_repo_regeneration_allowed_at_quota(
    ...
    'WorkflowFix selected' → 'Failed to queue fixes' once the quota was reached).
    """
...
def test_generate_fixes_for_repo_replaces_existing_fixes(
    ...
    assert db.get(WorkflowFix, fix_id) is None
...
def test_deliver_for_workflow_queues_batch(
    ...
    with patch(
        ...
        response = client.post(
            f"{settings.API_V1_STR}/workflow/fixes/{ready_fix.id}/deliveries",
            headers=superuser_token_headers,
        )
...
def test_deliver_for_workflow_unknown_id_returns_404(
    ...
    with patch("app.workers.tasks.fix_delivery.deliver_fixes_batch.delay"):
        response = client.post(
            f"{settings.API_V1_STR}/workflow/fixes/{uuid.uuid4()}/deliveries",
            headers=superuser_token_headers,
        )
...
def test_deliver_for_repo_not_found_returns_404(
    ...
    with patch("app.workers.tasks.fix_delivery.deliver_fixes_batch.delay"):
        response = client.post(
            f"{settings.API_V1_STR}/workflow/repositories/{uuid.uuid4()}/deliveries",
            headers=superuser_token_headers,
        )
...
def test_generate_fixes_for_repo_preserves_delivered_and_skips_in_task(
    ...
    with patch("app.api.routes.workflow_fixes.run_fix_generation.delay"):
        response = client.post(
            f"{settings.API_V1_STR}/workflow/repositories/{repo.id}/fixes",
            headers=superuser_token_headers,
        )
    ...
    assert response.status_code == 202
    db.expire_all()
    assert db.get(WorkflowFix, fix_id) is not None
    assert db.get(WorkflowFix, fix_id).status == FixStatus.delivered
...
def test_sync_pr_status_repo_not_found(
    ...
    response = client.post(
        f"{settings.API_V1_STR}/workflow/repositories/{uuid.uuid4()}/pull-requests/sync",
        headers=superuser_token_headers,
    )
    assert response.status_code == 404
...
def test_regenerate_for_repo_replaces_fixes_and_queues_per_workflow(
    ...
    with patch("app.api.routes.workflow_fixes.run_fix_generation.delay") as mock_delay:
        response = client.post(
            f"{settings.API_V1_STR}/workflow/repositories/{repo.id}/fixes/regenerate",
            headers=superuser_token_headers,
        )
    ...
    for wf in (wf1, wf2):
        new_fix = db.exec(
            sql_select(WorkflowFix).where(WorkflowFix.workflow_file_id == wf.id)
        ).one()
...
def test_regenerate_for_repo_skips_merged_pr_fix(
    ...
    assert response.json()["queued"] == 1
    db.expire_all()
    assert db.get(WorkflowFix, merged_fix_id) is not None
    assert db.get(PullRequest, pr_id) is not None
    assert db.get(WorkflowFix, other_fix_id) is None
...
def test_regenerate_for_repo_skips_in_flight_fix(
    ...
    assert response.json()["queued"] == 1
    db.expire_all()
    assert db.get(WorkflowFix, pending_fix_id) is not None
...
def test_regenerate_for_repo_keeps_closed_pr_referenced_by_surviving_fix(
    ...
    db.expire_all()
    assert db.get(PullRequest, pr_id) is not None
    surviving_fix = db.get(WorkflowFix, surviving_fix_id)
...
def test_regenerate_for_repo_keeps_closed_pr_referenced_by_other_engine(
    ...
    ``ON DELETE SET NULL`` — so sweeping a closed record that only ``WorkflowFix`` was
...
def test_regenerate_for_repo_not_found(
    ...
    response = client.post(
        f"{settings.API_V1_STR}/workflow/repositories/{uuid.uuid4()}/fixes/regenerate",
        headers=superuser_token_headers,
    )
    assert response.status_code == 404
...
def test_regenerate_for_workflow_replaces_fix(
    ...
    with patch("app.api.routes.workflow_fixes.run_fix_generation.delay") as mock_delay:
        response = client.post(
            f"{settings.API_V1_STR}/workflow/fixes/{old_fix_id}/regenerate",
            headers=superuser_token_headers,
        )
    ...
    assert db.get(WorkflowFix, old_fix_id) is None
    new_fix = db.exec(
        sql_select(WorkflowFix).where(WorkflowFix.workflow_file_id == workflow_file.id)
    ).one()
    assert new_fix.status == FixStatus.pending
...
def test_regenerate_for_workflow_retries_needs_manual_work_issue(
    ...
    with patch("app.api.routes.workflow_fixes.run_fix_generation.delay") as mock_delay:
        response = client.post(
            f"{settings.API_V1_STR}/workflow/fixes/{ready_fix.id}/regenerate",
            headers=superuser_token_headers,
        )
...
def test_regenerate_for_workflow_rejects_in_flight_fix(
    ...
    response = client.post(
        f"{settings.API_V1_STR}/workflow/fixes/{fix_id}/regenerate",
        headers=superuser_token_headers,
    )
...
def test_regenerate_for_workflow_keeps_shared_closed_pr(
    ...
    db.expire_all()
    assert db.get(PullRequest, pr_id) is not None
    sibling_fix = db.get(WorkflowFix, sibling_fix_id)
...
def test_regenerate_for_workflow_allowed_at_quota(
    ...
    with patch("app.api.routes.workflow_fixes.run_fix_generation.delay") as mock_delay:
        response = client.post(
            f"{settings.API_V1_STR}/workflow/fixes/{target_fix_id}/regenerate",
            headers=headers,
        )
...
def test_regenerate_for_workflow_not_found(
    ...
    response = client.post(
        f"{settings.API_V1_STR}/workflow/fixes/{uuid.uuid4()}/regenerate",
        headers=superuser_token_headers,
    )
    assert response.status_code == 404
...
def test_regenerate_failed_fix_in_place(
    ...
    with patch("app.api.routes.workflow_fixes.run_fix_generation.delay") as mock_delay:
        resp = client.post(
            f"{settings.API_V1_STR}/workflow/fixes/{fix.id}/retry",
            headers=superuser_token_headers,
        )
